...
## What did John Snow Make?

{{< pdf https://johnsnow.matrix.msu.edu/documentUploads/15-78-1DB/15-78-1DB-22-1855-01-SnowMCC2-map1-rev.pdf >}}

And today, we can make something like this with code:

```{r}
#| label: fig-cholera-map
#| fig-cap: "John Snow's Cholera Map With our Model Superimposed"
#| echo: false
#| 
library(HistData)
library(ggplot2)
...
# Flatten the list of polygon vertex data frames into one df with a pump ID
polygons_df <- bind_rows(
  lapply(seq_along(Snow.polygons), function(i) {
    data.frame(Snow.polygons[[i]], pump = i)
  })
)

ggplot() +
  # Voronoi (Thiessen) regions, one fill color per pump catchment area
  geom_path(data = polygons_df, aes(x = x, y = y, group = pump), color = "grey50", linetype = "dashed", linewidth = 2) + 
  # geom_polygon(
  #   data = polygons_df,
  #   aes(x = x, y = y, group = pump, fill = factor(pump)),
  ...
  geom_point(
    data = Snow.deaths,
    aes(x = x, y = y),
    color = "firebrick", size = 1.3, alpha = 0.8
  ) +
  # Pumps
  geom_point(
    data = Snow.pumps,
    aes(x = x, y = y),
    shape = 17, size = 4, color = "steelblue4"
  ) +
  geom_text(
    data = Snow.pumps,
    aes(x = x, y = y, label = label),
    vjust = -1, size = 3, color = "steelblue4", fontface = "bold"
  ) +
  coord_equal() +
  guides(fill = "none") +
  labs(
    title = "John Snow's 1854 Cholera Map",
    subtitle = "Streets, deaths, pumps, and Voronoi catchment areas per pump",
    x = NULL, y = NULL
  ) +
  theme_minimal() +
  theme(
...
Can we use Kandinsky's very idiosyncratic interpretation of triangles to create a diagram that works to measure Proximity? What could such a diagram look like? 
...
The *Voronoi Diagram* is possibly easier to interpret: it splits up (a 2D plane ) into disjoint areas which are closest to one-and-only-one of a set of points; i.e it splits the plane into disjoint **neighbourhoods**. 

A Dual to the Voronoi is the *Delaunay Triangulation*: this diagram breaks up the plane into discrete closed-off **triangular sub-spaces**. These subspaces usually are built up based on specific properties we are interested in: temperature, density, slope, vibration/center of gravity etc. Any quantity that varies continuously over a 2D space, cane be modelled using a set of *finite elements* created by the Delaunay Triangulation. 

## Where are these applied? 
See and exhaustive list below! 

## How do we Construct these Diagrams?
...
-   Wasn't that [a very Kandinsky-like thing]{style="background-color:yellow;"} to do, even for an
    algorithm?
...
To be written up!!
<https://orangedatamining.com/widget-catalog/visualize/silhouetteplot/>
...
1. The John Snow Archive and Research Companion. <https://johnsnow.matrix.msu.edu/>
1. Mafe Callejón.(21 February 2022). *Masters series: Recreating John Snow's cholera map: See an interactive version of John Snow's iconic cholera map* <https://flourish.studio/blog/masters-john-snow/> 
...
1. Rudyard Kipling. *How Fear Came*. <https://www.kiplingsociety.co.uk/tale/how-fear-came.htm>
