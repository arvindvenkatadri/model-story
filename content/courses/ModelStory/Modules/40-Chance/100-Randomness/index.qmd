...
us an aggregate [**Poisson distribution**](https://math.oxford.emory.edu/site/math117/connectingPoissonAndBinomial/)
for the occurrence of traffic, with a mean of $\large{\lambda = np}$.

Is this an example of Complexity? Not quite, or weakly at best. Why?

- The individual AGENTs do not interact with each other, like in Prisoners' Dilemma. Their choices are independent, not related. They do not decide to drive or not drive based whether others are driving out or not. They do not communicate with each other, or coordinate their actions.
- The individual AGENTs do not have any memory of their past actions, or the actions of others. They do not learn from their past experiences, or adapt to the changing environment. 
- There is no feedback; each choice is made regardless of prior experiences. 

Hence thus is a weakly complex system, and not a strongly complex system. But it is a **random or stochastic process**, which we need to understand and try to work with.

Complexity ***would*** emerge in this situation if:

- Agents acted based on Congestion / Road Occupancy
- Seeing more cars yesterday makes them less likely to drive today
- Social influences and clustering; e.g. car pooling.
- If all of these affect future choices ( drive / don't drive). 

In this case we would see examples of **emergent behavior** and **self-organization**, which are hallmarks of Complexity:

- Traffic Waves
- Ghost Traffic Jams
- Even Power Law distributions of vehicles across streets
- Phase transitions from Jammed to free-flow.

## So is Randomness a Design Tool?
...
In the Poisson Process above, we saw randomness and how different sequences of events can be generated by a large number of independent agents. Are we then at the **mercy of randomness**? Or can we use randomness as a tool in Design? Can we use randomness to create fairness, or to create surprise, or to create novelty?

Well, mangoes are far more interesting than cars!!!
...
6.  Or the last person to bat or bowl, or whatever was the cool thing in
    the game...
7.  What strategy did you use when playing "Rock, Paper, Scissors"? Were you always "steady as a rock", or "variable as traffic"? You didn't stick with just one of Rock, Paper, or Scissors. But
chose one at.....random.
...
[***So somehow it was all...fair, because it was "random"***]{style="background-color:yellow;"}.
...
Let us throw some chalk/paper pieces on the floor at random and see what happens!!
