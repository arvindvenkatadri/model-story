...
Hence a total of **8 Tests** were created by 8 pairs of students, and each team tested the remaining 15 students ( [Due to COVID restrictions, this testing was carried out entirely online on MS Teams, using individual breakout rooms for the Test Teams.]{style="background-color:yellow;"} )

The data were entered into a Google Sheet and [the STM scores were converted to *percentages* so as to be comparable across WL]{style="background-color:yellow;"}. 
...
Joint effects were not considered for this preliminary class. 


## Data

The collated data was placed in an [Excel sheet](RandomTesters.xlsx). This was read first:
...
null_dist_words <-
  do(1000) * diff(mean(stm_words$list_scores ~ shuffle(stm_words$list), data = stm_words))
